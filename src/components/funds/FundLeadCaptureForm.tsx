...
interface FundLeadCaptureFormProps {
  result: EligibilityResult;
  schemeName: string;
}

export default function FundLeadCaptureForm({
  result,
  schemeName,
}: FundLeadCaptureFormProps) {
  ...
  if (submitted) {
    const waText = encodeURIComponent(
      `你好，我已完成${schemeName}資格問卷（${result.title}），姓名：${name}，想了解申請安排`,
    );
    return (
  ...
  return (
    ...
    >
      ...
      <button
        type="submit"
        disabled={loading}
        className="mt-3 w-full rounded-xl bg-violet-600 py-3 text-sm font-bold text-white hover:bg-violet-700 disabled:opacity-60"
      >
        {loading ? "提交中…" : "提交查詢"}
      </button>
    </form>
